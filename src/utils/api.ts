...
export const getAttendanceRecordByListId = (listId: number) =>
  api.get(`/attendanceRecord/list/${listId}`);
export const checkAttendance = (id: number, data: Record<string, unknown>) =>
...
export const searchVocabList = (query: string) =>
  api.get(`/vocabList/search?query=${query}`);
export const getVocabByListId = (id: number) => api.get(`/vocab/list/${id}`);
export const createVocab = (id: number, data: Record<string, unknown>) =>
  api.post(`/vocab/list/${id}`, data);
...
export const createVocabExternal = (
  id: number,
  data: Record<string, unknown>
) => api.post(`/vocab/external/list/${id}`, data);
export const sortVocabs = (id: number, column: string, order: string) =>
  api.get(`/vocab/list/${id}/sort?column=${column}&order=${order}`);
export default api;
